...
# index number, name, latitud, longitude
for index in range(5):
    print(str(index) + ", "
        + json_dict["data"]["search"]["searchResults"][index]["title"] + ", " 
        + str(json_dict["data"]["search"]["searchResults"][index]["lat"]) + ", " 
        + str(json_dict["data"]["search"]["searchResults"][index]["lon"]))
# (base) chino@chino-Legion-5-15IAH7H:~/LoRaWorks_230822/test_mlit-dpf-api$ python ./test_file_read_json-1.py 
# 0, 成田市古込１−１成田国際空港第２旅客ターミナルビル地下１階 日本医科大学成田国際空港クリニック, 35.77365489125021, 140.38844830973645
# 1, 成田国際空港警察署, 35.78104658, 140.362453
# 2, 成田国際空港, 35.761558, 140.387063
# 3, 成田2, 35.78361, 140.35889
# 4, 駒井野橋, 35.78529, 140.3714
